...
public static partial class Tasks
{
    ...
    {
        int maxSalary = Emps.Max(x => x.Salary);
        return Emps.Where(x => x.Salary == maxSalary);
    }
    ...
    {
         return from e in Emps join d in Depts on e.Deptno equals d.Deptno select new { e.Ename, e.Job, d.Dname };
    }
    ...
    {
        return Emps.GroupBy(x => x.Job).Select(g => new { Praca = g.Key, LiczbaPracownikow = g.Count() });
    }
    ...
    {
        if(Emps.Where(x => x.Job == "Backend programmer") == null)
        {
            return false;
        }
        else
    ...
    {
        return Emps.Where(x => x.Job == "Frontend programmer").OrderByDescending(x => x.HireDate).FirstOrDefault();
    }
    ...
    {
        var f = Emps.Select(e => new { e.Ename, e.Job, e.HireDate });
        var s = new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } };
        return f.Union<object>(s);
    }
    ...
    {
        return Emps.GroupBy(e => e.Deptno).Join(Depts.Where(d => Emps.Any(e => e.Deptno == d.Deptno)), emp => emp.Key, dept => dept.Deptno, (emp, dept) => new { name = dept.Dname, numOfEmployees = emp.Count() })
    ...
    {
        int res = arr.GroupBy(x => x).Single(x => x.Count() % 2 != 0).Key;
        return res;
    }
    ...
    {
        return Depts.GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emp) => new { department = dept, numOfEmployees = emp.Count() })
            .Where(r => r.numOfEmployees == 5 || r.numOfEmployees == 0)
            .OrderBy(r => r.department.Dname)
